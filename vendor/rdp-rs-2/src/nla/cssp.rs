...
pub fn read_public_key(stream: &[u8]) -> RdpResult<Vec<u8>> {
    let certificate = Certificate::from_der(stream).map_err(|e| {
    ...
    certificate
        ...
        .ok_or_else(|| {
            Error::RdpError(RdpError::new(
                RdpErrorKind::InvalidData,
                "Invalid server public key bit string",
            ))
        })
}
...
pub fn cssp_connect<S: Read + Write>(link: &mut Link<S>, authentication_protocol: &mut dyn AuthenticationProtocol, restricted_admin_mode: bool) -> RdpResult<()> {
    ...
    let public_key = read_public_key(&certificate_der)?;

    // Now we can send back our challenge payload wit the public key encoded
    let challenge = create_ts_authenticate(client_challenge, security_interface.gss_wrapex(&public_key)?);
    ...
    if BigUint::from_bytes_le(&inc_pub_key) != BigUint::from_bytes_le(&public_key) + BigUint::new(vec![1]) {
